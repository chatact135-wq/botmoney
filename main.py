...
async def position_management_loop():
    """
    Resilient Trailing Engine:
    - Throttles updates and ensures stop-loss modifications clear broker distance/spread requirements.
    """
    global is_bot_running, global_connection
    print("Resilient Trailing Engine online...")
    ...
    while is_bot_running:
        try:
            if global_connection and is_bot_running:
                ...
                for pos in positions:
                    ...
                    # Safe broker-compliant trailing step ($2.00+ net profit to account for spread and stops level)
        ...
        # Throttled to 1.5 seconds to prevent rate-limiting or API flooding
...
async def startup_event():
    ...
    if not bot_task or bot_task.done():
        bot_task = asyncio.create_task(run_resilient_bot())
...
async def read_dashboard():
    ...
    <html>
    <head>
        <title>Gold Resilient System</title>
...
async def manual_place_buy():
    try:
        ...
        res = await connection.create_market_buy_order(symbol="XAUUSDm", volume=0.1, stop_loss=round(ask-30,2), take_profit=round(ask+25,2))
...
async def manual_place_sell():
    try:
        ...
        res = await connection.create_market_sell_order(symbol="XAUUSDm", volume=0.1, stop_loss=round(bid+30,2), take_profit=round(bid-25,2))
        return {"status": "success", "order": res}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
